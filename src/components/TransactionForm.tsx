...
import { ArrowRightLeft, HelpCircle, Check, Sparkles, Tag, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function TransactionForm({
  ...
  const [isExpanded, setIsExpanded] = useState(() => {
    const stored = localStorage.getItem('tx_form_expanded');
    return stored === 'true'; // Defaults to collapsed (false) if not found or explicitly false
  });

  const toggleExpand = () => {
    const nextVal = !isExpanded;
    setIsExpanded(nextVal);
    localStorage.setItem('tx_form_expanded', String(nextVal));
    if (nextVal) {
      setTimeout(() => {
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      if ((e.altKey && e.key === 'n') || (e.altKey && e.key === 'N')) {
        e.preventDefault();
        setIsExpanded(true);
        localStorage.setItem('tx_form_expanded', 'true');
        setTimeout(() => {
          payeeInputRef.current?.focus();
        }, 50);
      }
  ...
  return (
    <div 
      className={`bg-white rounded border border-[#e4e2d9] shadow-md sticky top-4 z-40 text-slate-800 transition-all duration-300 ${isExpanded ? 'p-5' : 'p-3.5'}`} 
      id="sticky-input-card"
    >
      {/* Dynamic Header Toggle Bar */}
      <button
        type="button"
        onClick={toggleExpand}
        className="w-full flex items-center justify-between text-left cursor-pointer outline-none select-none font-sans"
        aria-expanded={isExpanded}
        aria-label="Toggle Rapid Manual Input Panel"
      >
        <div className="flex items-center gap-3">
          <div className={`p-1.5 rounded-sm transition-colors duration-200 ${isExpanded ? 'bg-slate-800 text-white' : 'bg-[#eae8df] text-slate-600'}`}>
            <Sparkles size={13} className={isExpanded ? 'animate-pulse' : ''} />
          </div>
          <div>
            <h4 className="text-xs font-sans font-bold text-slate-800 flex items-center gap-1.5 leading-tight">
              Rapid Manual Input Panel
            </h4>
            <p className="text-[10px] font-mono text-slate-500 leading-tight">
              {isExpanded ? 'Quick transaction record sheet' : 'Click or hit Alt+N to expand input screen'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          {!isExpanded && (
            <span className="hidden sm:inline-block text-[10px] font-mono text-slate-400 bg-slate-50 border border-slate-100 rounded px-1.5 py-0.5">
              Alt + N
            </span>
          )}
          <div className="text-slate-500 mr-1 shrink-0">
        ...
        {isExpanded && (
          ...
          >
            <div className="pt-4 border-t border-slate-100 mt-4 space-y-4">
              <div className="flex justify-between items-center text-[10px] font-mono text-slate-400 select-none px-1">
                <span>Record transaction dynamically in-place</span>
                <span className="flex items-center gap-2">
                  <span><kbd className="bg-slate-50 border border-slate-200 text-slate-500 px-1 py-0.2 rounded-sm">Alt + N</kbd> focus</span>
                  <span><kbd className="bg-slate-50 border border-slate-200 text-slate-500 px-1 py-0.2 rounded-sm">Enter</kbd> submit</span>
                </span>
              </div>
              ...
              >
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <select
                    ...
                    className="w-full text-xs font-sans bg-white border border-[#d2d0c5] text-slate-800 rounded-sm p-2.5 outline-none focus:ring-2 focus:ring-sky-500/10 focus:border-sky-500 cursor-pointer shadow-sm"
                    tabIndex={1}
                  >
                    {accounts.map(acc => (
                ...
                <div className="md:col-span-2 relative space-y-1" ref={payeeSuggestionsRef}>
                  ...
                  <input
                    ...
                    className="w-full text-xs bg-white border border-[#d2d0c5] text-slate-800 placeholder-slate-400 rounded-sm p-2.5 outline-none focus:ring-2 focus:ring-sky-500/10 focus:border-sky-500 shadow-sm"
                    tabIndex={3}
                    autoComplete="off"
                  />
              ...
              <div className="flex flex-wrap gap-4 pt-3.5 border-t border-slate-200 text-xs">
                <label className="flex items-center gap-2 text-slate-655 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={reconciled}
                    ...
                    tabIndex={8}
                  />
                  <span className="font-mono text-[11px] text-slate-550">Mark Reconciled immediately</span>
                </label>
                ...
                <label className="flex items-center gap-2 text-slate-655 cursor-pointer select-none">
                  <input
                    ...
                    className="rounded-sm border-slate-300 text-sky-600 focus:ring-sky-500/20 bg-white"
                  />
                  <span className="font-mono text-[11px] text-slate-550 flex items-center gap-1">
                    <ArrowRightLeft size={12} className="text-slate-505" />
